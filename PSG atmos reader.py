...
def atmosatm(model, location,filebase = ''):
	...
	if len(filebase):
		...
		config_name=dir_root+"bioverse-atmos-integration/psg_configs/"+model_file
...
def psgspec(model, location,config=[], showplot=True):
	...
	model_file="%s_cfg.txt" % model
	config_name=dir_root+"Alien_Earths/bioverse-atmos-integration/psg_configs/"+model_file
	target=dir_root+"Alien_Earths/bioverse-atmos-integration/"+model_file
	...
	dest_path=dir_root+"Alien_Earths/bioverse-atmos-integration/psg_output/"+"%s_rad.txt" % model
	# More file shuffling
	...
	if showplot:
		...
		fw = open("%s_rad.txt" % model); lines = fw.readlines(); fw.close()
		for line in lines:
			if line[0]!='#': break
		...
		data = np.genfromtxt("%s_rad.txt" % model)
		...
		plt.tight_layout()
		figname=dir_root+"Alien_Earths/bioverse-atmos-integration/figures/"+"%s_rad.png" % model
		plt.savefig(figname)
		plt.show()
	os.remove("%s_cfg.txt" % model)
	os.remove("%s_rad.txt" % model)
	
location="office" #this is just a variable I use personally
