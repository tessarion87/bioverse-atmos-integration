...
def atmosatm(model, location,filebase = ''):
	...
	if location=="office":
		dir_root="/home/tessa/Alien Earths/"
	if len(filebase):
		model=model.split('/')[1]
...
def psgspec(model, location,config=[], showplot=True):
	...
	model_file="%s_cfg.txt" % model
	config_name=dir_root+"bioverse-atmos-integration/psg_configs/"+model_file
	target=dir_root+"bioverse-atmos-integration/"+model_file
	...
	dest_path=dir_root+"bioverse-atmos-integration/psg_output/"+"%s_rad.txt" % model
	# More file shuffling
	...
	if showplot:
		...
		plt.tight_layout()
		figname=dir_root+"bioverse-atmos-integration/figures/"+"%s_rad.png" % model
		plt.savefig(figname)
		plt.show()
	os.remove("%s_cfg.txt" % model)
	os.remove("%s_rad.txt" % model)
	
location="home" #this is just a variable I use personally
model_list=glob.glob('sample atmos results/earth*')
